...
import { IResource, IField } from './Admin'
...
class ListResource extends React.Component<Props, null> {
    render() {
        ...
        const columns: any[] = resource.fields
            ...
            .map(field => ({
                Header: field.type === 'reference' ? field.references.name : field.name,
                accessor: field.name,
                ...
                Cell:
                    field.type === 'reference'
                        ? row => {
                              return (
                                  <Link
                                      to={`${field.references.url}/${
                                          row.original[field.name]
                                      }`}
                                  >
                                      {resources[field.references.name].find(resource => resource.id === row.original[field.name])[field.references.descriptiveField]}
                                  </Link>
                              )
                          }
                        : undefined,
        ...
        columns.push({
            ...
            width: 50,
            Cell: props => {
                return <Link to={`./${props.original.id}`}>Edit</Link>
            },
        })
            ...
            <ReactTable
                ...
                SubComponent={
                    resource.fields.some(
                        resource => resource.showInList === false
                    )
                        ? row => {
